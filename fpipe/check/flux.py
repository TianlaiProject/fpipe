...
def load_catalogue(nvss_path_list, nvss_range, flux_key, name_key, 
    ...
    for nvss_path in nvss_path_list:
        ...
        first_major = nvss_cat['FIRST_MAJOR']
        nvss_sel *= first_major < max_major_axis
...
def iter_nvss_flux_tod(fdata, nvss_cat, fwhm=None, beam_list=None, max_dist=3):
    ...
    for beam in beam_list:
        ...
        for nvss_idx in nvss_on:
            ...
            pp = p[t_sel, :]
            rr_min = rr.min()
            flux_model = np.sum(beam_model(rr, sigma[i]) * nvss_flx, axis=1)
...
def output_source(file_root, file_name_list, output_path, nvss_path_list,
    ...
    for file_name in file_name_list:
        ...
        if map_info is not None:
            ...
            freq_sel = (freq_imap > fdata.freq.min()) * (freq_imap < fdata.freq.max())

        if nvss_map_info is not None:
        ...
        with h5py.File(output_path + output_name + '.h5', 'w') as f:
            ...
            for oo in iter_nvss_flux_tod(fdata, nvss_cat, fwhm, max_dist=0.5):
                ...
                if map_info is not None:
                    _r = get_map_spec([imap[freq_sel], pixs, nside, nmap[freq_sel]], 
                            nvss_ra, nvss_dec, freq_imap[freq_sel], mJy=True)
                    if _r is not None:
                        spec, p_ra, p_dec, error = _r
                        flux_from_map = np.ma.median(spec)
                        _pix = hp.ang2pix(nside_nvss, nvss_ra, nvss_dec, lonlat=True)
                        if np.any(pixs_nvss == _pix):
                            _pix = np.where(pixs_nvss == _pix)[0][0]
                            flux_model_from_map = nvss_map[_pix]
                        else:
                            flux_model_from_map = 0
                    else:
                        flux_from_map = 0
                        flux_model_from_map = 0
                ...
                print 'Beam %02d %s [%10.5f %10.5f] TOD:%10.5f +- %10.5f, MAP:%10.5f, MAP Model:%10.5f'%(
                        beam, nvss_name, nvss_ra, nvss_dec, flux_measured[rr.argmin()][0], 
                        flux_measured_rms[rr.argmin()][0], flux_from_map,flux_model_from_map)
                ...
                f['B%02d/%s/MAPS_FLUX'%(beam, nvss_name)] = flux_from_map
                f['B%02d/%s/MAPS_NVSS'%(beam, nvss_name)] = flux_model_from_map
...
def check_flux(tod_flux_files, beam_list=None,
    ...
    for file_name in tod_flux_files:
        ...
        with h5py.File(file_name, 'r') as f:
            if beam_list is None:
                beam_list = [int(x[1:]) for x in f.keys()]
        ...
        for flux_path in flux_path_list[1:]:
            ...
            with h5py.File(flux_path, 'r') as f:
                for i in beam_list: # iter through ref
                    ...
                    for s in f0['B%02d'%i].keys(): # iter through ref
                        for j in beam_list: # search in individual files
                            ...
                            try:
                                ...
                                pass
            yield label, np.array(flux_ref0), np.array(flux_fast), None
